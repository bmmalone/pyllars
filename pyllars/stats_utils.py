...
def sample_dirichlet_multinomial(dirichlet_alphas:np.ndarray, num_samples:int) -> np.ndarray:
    ...
    Parameters
    ----------
    dirichlet_alphas : numpy.ndarray
        The concentration parameters for the Dirichlet distribution

    num_samples : int
        The number of samples to draw for the multinomial

    Returns
    -------
    counts : np.ndarray
        The number of counts from each level in the multinomial. This will be
        the same size as `dirichlet_alphas`.
    """
